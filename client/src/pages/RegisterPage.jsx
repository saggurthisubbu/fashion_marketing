...
export const RegisterPage = ({ onClose }) => {
  const { loginUser, API_BASE_URL, showToast, user, checkoutRedirectPending } = useShop();
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setSuccess(isCheckoutRedirect ? 'Account created! Taking you to checkout...' : successMsg);
      ...
      setTimeout(() => {
        ...
        window.dispatchEvent(new PopStateEvent('popstate'));
        onClose?.();
        // Note: App.jsx detects checkoutRedirectPending and auto-opens checkout
      }, 1200);
    } catch (err) {
